...
# везде поставил max_iter = 4000, потому что требовало

fig, axes = plt.subplots(1, 2, figsize=(10, 3))
for model, ax in zip([LinearSVC(max_iter=4000), LogisticRegression(max_iter=4000)], axes):
    clf = model.fit(X, y)
...
logreg100 = LogisticRegression(C=100, max_iter=4000).fit(X_train, y_train)
...
logreg001 = LogisticRegression(C=0.01, max_iter=4000).fit(X_train, y_train)
...
for C, marker in zip([0.001, 1, 100], ['o', '^', 'v']):
    lr_l1 = LogisticRegression(C=C, penalty="l2", max_iter=4000).fit(X_train, y_train)
